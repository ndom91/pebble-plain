...
# pebble publish only uses listing metadata when creating a new app. For an
# existing app, edit the public listing in the RePebble appstore dashboard.
...
UPLOAD_SCREENSHOTS="${UPLOAD_SCREENSHOTS:-0}"

if [ -z "$SOURCE_URL" ]; then
  printf 'SOURCE_URL is required, for example:\n  SOURCE_URL=https://github.com/<owner>/<repo> %s\n' "$0" >&2
  exit 1
fi
...
if [ "$UPLOAD_SCREENSHOTS" = "1" ]; then
  ...
  publish_args+=(--screenshots "${screenshots[@]}")
elif [ "$#" -gt 0 ]; then
  printf 'Screenshot paths were provided, but screenshot upload is disabled. Set UPLOAD_SCREENSHOTS=1 to upload them.\n' >&2
  exit 1
fi

pebble publish "${publish_args[@]}"
